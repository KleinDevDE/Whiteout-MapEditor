<template>
    <div class="relative h-screen w-screen overflow-hidden bg-white">
        <!-- Toolbar -->
        <div
            class="absolute z-10 m-3 rounded-lg border border-gray-500 bg-gray-700/70 p-3 shadow-md shadow-gray-700"
        >
            ...
            <div class="mb-2 flex flex-wrap gap-2">
                <button
                    ...
                    :key="opt.id"
                    class="btn rounded border px-2 py-1 text-sm"
            ...
            <div class="mt-1 text-sm font-bold text-gray-100">Color</div>
            ...
            <div class="mb-5 flex gap-2">
                <button class="btn px-2 py-1" @click="clearAll">Clear</button>
                <button class="btn px-2 py-1" @click="centerView">
                    Center
                </button>
            </div>
...
const toolOptions = Object.values(STAMPS).map((s) => ({
    id: s.id,
    label: s.label,
}));
...
const inBounds = (x: number, y: number) =>
    x >= 0 && x < WORLD_W + 1 && y >= 0 && y < WORLD_H + 1;

// Editor uses top-left origin; game uses bottom-left with axes flipped
const toGameCoords = (x: number, y: number) => ({
    x: WORLD_H - y,
    y: WORLD_W - x,
});
const toEditorCoords = (x: number, y: number) => ({
    x: WORLD_W - y,
    y: WORLD_H - x,
});
...
function applyStampAt(
    ...
    if (!erase) {
        ...
        Session.placedTiles.value = Session.placedTiles.value.filter((o) => {
            ...
            const hit = stamp2.shape.some(
                (s) =>
                    o.origin.x + s.x === gPos.x && o.origin.y + s.y === gPos.y
            );
...
function canPlaceStamp(gx: number, gy: number) {
    ...
    if (!stamp)
        return { ok: false, hits: [] as Array<{ x: number; y: number }> };
...
function visibleBounds() {
    ...
    const x2 = Math.min(
        WORLD_W,
        Math.ceil(((c.clientWidth - offset.value.x) * inv) / TILE) + 1
    );
    const y2 = Math.min(
        WORLD_H,
        Math.ceil(((c.clientHeight - offset.value.y) * inv) / TILE) + 1
    );
    return { x1, y1, x2, y2 };
}
...
function onDown(e: MouseEvent) {
    if (e.button === 1 || e.shiftKey) {
        ...
        hoverX.value = gx;
        hoverY.value = gy;
        hoverWorldX.value = world.x;
        hoverWorldY.value = world.y;
...
function onMove(e: MouseEvent) {
    ...
    if (mode.value === 'paint') {
        ...
        const { ok } = canPlaceStamp(gx, gy);
        c.style.cursor =
            ok || toolMode.value === 'erase' ? 'crosshair' : 'not-allowed';
        draw();
    }
}
function onUp() {
...
function draw() {
    ...
    for (let x = x1; x <= x2 + 1; x++) {
        ctx.beginPath();
        const X = crispX(x);
        ctx.moveTo(X, y1 * TILE);
        ctx.lineTo(X, (y2 + 1) * TILE);
        ctx.stroke();
    }
    for (let y = y1; y <= y2 + 1; y++) {
        ctx.beginPath();
        const Y = crispY(y);
        ctx.moveTo(x1 * TILE, Y);
        ctx.lineTo((x2 + 1) * TILE, Y);
        ctx.stroke();
    }
    ...
    if (hoverX.value != null && hoverY.value != null) {
        ...
        if (stamp) {
            ...
            if (
                stamp.bannerRange !== undefined &&
                stamp.bannerRange > 0 &&
                tiles.value.get(keyOf(hoverX.value, hoverY.value)) === undefined
            ) {
                ...
                ctx.fillStyle = '#60a5fa'; // blaues Overlay
                for (
                    let dy = -stamp.bannerRange;
                    dy <= stamp.bannerRange;
                    dy++
                ) {
                    for (
                        let dx = -stamp.bannerRange;
                        dx <= stamp.bannerRange;
                        dx++
                    ) {
                        const x = hoverX.value + dx,
                            y = hoverY.value + dy;
                        if (!inBounds(x, y)) continue;
                        const k = keyOf(x, y);
                        const prev = bannerOverlay.value.get(k) ?? 0;
                ...
                for (
                    ...
                    y <= hoverY.value + stamp.bannerRange;
                    y++
                ) {
                    for (
                        let x = hoverX.value - stamp.bannerRange;
                        x <= hoverX.value + stamp.bannerRange;
                        x++
                    ) {
                        const cov = bannerOverlay.value.get(keyOf(x, y)) ?? 0;
                        if (cov < 1) continue;
